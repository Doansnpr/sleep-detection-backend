...
let jawabanList = [
    ...
    {id:4, pertanyaanId:2, tipe:'range',  rangeMin:18,  rangeMax:90,    unit:'tahun',      label:'Usia Dewasa'},
    {id:5, pertanyaanId:4, tipe:'range',  rangeMin:1,   rangeMax:12,    unit:'jam',        label:'Durasi Tidur'},
    {id:6, pertanyaanId:5, tipe:'range',  rangeMin:1,   rangeMax:10,    unit:'',           label:'Skala Kualitas'},
    {id:7, pertanyaanId:6, tipe:'range',  rangeMin:0,   rangeMax:120,   unit:'menit/hari', label:'Aktivitas Fisik'},
    {id:8, pertanyaanId:7, tipe:'range',  rangeMin:1,   rangeMax:10,    unit:'',           label:'Skala Stres'},
    {id:9, pertanyaanId:9, tipe:'range',  rangeMin:0,   rangeMax:25000, unit:'langkah',    label:'Langkah Harian'},
];
let nextId = 10;
...
function getJawabanByQ(qid) { return jawabanList.find(j => j.pertanyaanId === qid); }

// ============ STATS ============
function updateStats() {
    const sel = jawabanList.filter(j => j.tipe === 'select');
    const rng = jawabanList.filter(j => j.tipe === 'range');
    const totalOpsi = sel.reduce((acc, j) => acc + (j.opsi ? j.opsi.split('\n').filter(Boolean).length : 0), 0);
    document.getElementById('totalJawaban').textContent = jawabanList.length;
    document.getElementById('totalSelect').textContent  = sel.length;
    document.getElementById('totalRange').textContent   = rng.length;
    document.getElementById('totalOpsi').textContent    = totalOpsi;
}
...
function renderList() {
    ...
    let filtered = jawabanList.filter(j => {
        const q = getQuestion(j.pertanyaanId);
        if (!q) return false;
        const matchSearch = q.name.toLowerCase().includes(search);
        const matchType   = !typeFilter || j.tipe === typeFilter;
        return matchSearch && matchType;
    });

    document.getElementById('countBadge').textContent = filtered.length + ' jawaban';
...
function saveModal() {
    ...
    const data = { pertanyaanId: qid, tipe: q.type };

    if (q.type === 'select') {
        const opsi = document.getElementById('opsiJawaban').value.trim();
        if (!opsi) { showToast('Isi minimal satu opsi jawaban!', false); return; }
        data.opsi = opsi;
    } else {
        ...
        data.rangeMin = parseFloat(min);
        data.rangeMax = parseFloat(max);
        data.unit     = document.getElementById('rangeUnit').value.trim();
        data.label    = document.getElementById('rangeLabel').value.trim();
    }

    if (id) {
        const idx = jawabanList.findIndex(x => x.id === parseInt(id));
        jawabanList[idx] = { ...jawabanList[idx], ...data };
        showToast('Jawaban berhasil diperbarui!', true);
    } else {
        jawabanList.push({ id: nextId++, ...data });
        showToast('Jawaban berhasil ditambahkan!', true);
    }
...
function confirmDel() {
    jawabanList = jawabanList.filter(j => j.id !== deleteTarget);
